...
p3 <- function(object,sublabel=FALSE,keep_unknown=TRUE){
  if(sublabel){
    ...
    if(keep_unknown){
      p <- object@coverage %>% melt("Div") %>% mutate(group = stringr::str_remove(variable,"/.*$")) %>% mutate(group = stringr::str_remove(group,"\\?")) %>% filter(group %in% c("DNA","LINE","LTR","SINE","Unknown")) %>% ggplot(aes(x=Div,y=value/object@genome_size*100,fill=group)) + geom_bar(stat="identity")+cowplot::theme_cowplot()+scale_fill_brewer(palette="Accent")+ labs(x="Kimura substitution level (CpG adjusted)",y="Percent of genome (%)",title = object@species)
    }else{
      p <- object@coverage %>% melt("Div") %>% mutate(group = stringr::str_remove(variable,"/.*$")) %>% mutate(group = stringr::str_remove(group,"\\?")) %>% filter(group %in% c("DNA","LINE","LTR","SINE")) %>% ggplot(aes(x=Div,y=value/object@genome_size*100,fill=group)) + geom_bar(stat="identity")+cowplot::theme_cowplot()+scale_fill_brewer(palette="Accent")+ labs(x="Kimura substitution level (CpG adjusted)",y="Percent of genome (%)",title = object@species)
    }
  }
  return(p)
}
